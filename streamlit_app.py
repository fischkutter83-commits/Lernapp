...
if "login_success" not in st.session_state:
    st.session_state.login_success = False  # NEU: Flag für sicheren Rerun

# -------------------- LOGIN --------------------
if st.session_state.user is None or not st.session_state.login_success:
    ...
    with col1:
        if st.button("Einloggen"):
            if username in users and users[username]["password"] == password:
                ...
                st.session_state.login_success = True  # Flag setzen
    ...
    with col2:
        if st.button("Registrieren"):
            if username in users:
                ...
                st.error("❌ Bitte gültigen Benutzernamen und Passwort eingeben")
            else:
                users[username] = {"password": password}
                save_users(users)
                st.success("✅ Account erstellt – bitte einloggen")

    st.stop()  # Alles andere erst anzeigen, wenn User eingeloggt

# -------------------- Quiz-Funktionen --------------------
def generiere_mathe_aufgaben(klasse, anzahl):
    aufgaben = []
    ops = ["Plus", "Minus"] if klasse <= 2 else ["Plus", "Minus", "Mal", "Geteilt"] if klasse <=6 else ["Plus","Minus","Mal","Geteilt","Bruch","Potenz"]
...
def generiere_deutsch_aufgaben(klasse,anzahl):
    daten={1:("Plural",{"Hund":"Hunde","Katze":"Katzen"}),4:("Wortart",{"laufen":"Verb","Haus":"Nomen"}),7:("Synonym",{"groß":"riesig","klein":"winzig"})}
